...
test('every curated course is clearly external and links safely to an official provider',()=>{
  assert.equal((page.match(/EXTERNAL FREE LEARNING RESOURCE/g)||[]).length,4);
  ...
  assert.match(page,/https:\/\/nptel\.ac\.in\/courses\/127105234/);
  assert.equal((page.match(/target="_blank" rel="noopener noreferrer"/g)||[]).length,4);
  assert.equal((page.match(/Watch on provider site/g)||[]).length,4);
...
test('curation supplies original learning context and academy integration',()=>{
  for(const text of ['Prerequisites:','Learning outcomes:','Recommended study use:','Free-access status:']){
    assert.equal((page.match(new RegExp(text,'g'))||[]).length,4);
  }
  for(const path of ['tools/three-phase-power-calculator/','guides/vfd-fundamentals/','quiz-electrical.html#knowledge-check','guides/engineering-glossary/','guides/plc-troubleshooting/','quiz-plc.html#knowledge-check','resources/mtbf-mttr-data-collection-worksheet/','tools/mtbf-mttr-availability-calculator/','guides/root-cause-analysis-5-why/','guides/fmea-maintenance/']){
  ...
  assert.match(page,/calculations depend on complete records, consistent definitions/);
});
