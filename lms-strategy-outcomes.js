(()=>{
  ...
  const startExperiment=(skill,method)=>{
    ...
    const x=state();x.active={skillId:skill.id,method,baselineScore:pct(a[a.length-1]),baselineAttempts:a.length,startedAt:new Date().toISOString(),status:'Active',tasks:{}};save(x);return true;
  };
  const taskCard=(active,skill)=>{
    const tasks=studyTasks(skill,active.method),done=tasks.filter((_,i)=>active.tasks?.[i]).length;
    return `<div class="lms-item" style="margin-top:12px;align-items:start"><div class="lms-item-icon">${done}/${tasks.length}</div><div style="min-width:0"><h3>Study tasks before reassessment</h3><p>Work through these steps in order. Marking a task complete records study progress on this browser; the fresh quiz attempt remains the outcome measure.</p><div style="display:grid;gap:8px;margin-top:10px">${tasks.map((t,i)=>{const checked=!!active.tasks?.[i];return `<div style="border:1px solid rgba(145,225,255,.14);border-radius:10px;padding:11px;background:rgba(255,255,255,.015)"><div style="display:flex;gap:10px;align-items:flex-start"><button type="button" class="btn btn-secondary" data-strategy-task="${i}" aria-pressed="${checked}" style="padding:8px 10px;min-width:42px">${checked?'✓':'${i+1}'}</button><div><strong style="font-size:12px">${esc(t.title)}</strong><p style="margin:4px 0 0">${esc(t.detail)}</p>${t.href?`<a href="${esc(t.href)}" style="display:inline-block;margin-top:6px;color:var(--cyan);font-size:11px;font-weight:800">${esc(t.label)} →</a>`:''}</div></div></div>`}).join('')}</div></div><div><strong style="font-size:16px;color:var(--cyan)">${Math.round(done/tasks.length*100)}%</strong><p class="lms-local-note" style="margin-top:4px">study tasks</p></div></div>`;
  };
  const render=()=>{
