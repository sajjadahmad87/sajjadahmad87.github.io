(()=>{
  ...
  const completeIfReady=(x)=>{
    ...
    x.history.unshift({...x.active,afterScore:after,delta:after-before,completedAt:new Date().toISOString(),status:'Completed'});x.active=null;save(x);return x;
  };
  const prioritySkill=()=>{
  ...
  const startExperiment=(skill,method)=>{
    ...
    const x=state();x.active={skillId:skill.id,method,baselineScore:pct(a[a.length-1]),baselineAttempts:a.length,startedAt:new Date().toISOString(),status:'Active'};save(x);return true;
  };
  const render=()=>{
