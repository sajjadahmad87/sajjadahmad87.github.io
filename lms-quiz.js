(()=>{
  ...
  const mount=()=>{
    ...
    form.addEventListener('submit',e=>{
      ...
      const pct=Math.round(score/QUESTIONS.length*100);result.hidden=false;result.textContent=`Score: ${score}/${QUESTIONS.length} (${pct}%). ${pct>=80?'Strong result. Review any missed explanations before continuing.':'Review the explanations, revisit the linked guides, and try again when ready.'}`;result.focus();
    });
