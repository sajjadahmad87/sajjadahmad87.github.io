(()=>{
  ...
  const mount=()=>{
    ...
    form.addEventListener('submit',e=>{
      ...
      if(answered<QUESTIONS.length){result.hidden=false;result.textContent=`You answered ${answered} of ${QUESTIONS.length} questions. Complete all questions to save an attempt.`;return}
      ...
      const pct=Math.round(score/QUESTIONS.length*100);result.hidden=false;result.textContent=`Score: ${score}/${QUESTIONS.length} (${pct}%). ${pct>=80?'Strong result. Review any missed explanations before continuing.':'Review the explanations, revisit the linked guides, and try again when ready.'}`;
    });
