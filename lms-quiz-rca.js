(()=>{
  ...
  const mount=()=>{
    ...
    form.addEventListener('submit',e=>{
      ...
      const result=form.querySelector('[data-lms-rca-result]');if(answered<QUESTIONS.length){result.hidden=false;result.textContent=`You answered ${answered} of ${QUESTIONS.length} questions. Complete all questions to save an attempt.`;const firstUnanswered=QUESTIONS.findIndex((_,i)=>fd.get('q'+i)===null);const firstInput=firstUnanswered>=0?form.querySelector(`input[name="q${firstUnanswered}"]`):null;if(firstInput)firstInput.focus();return}
      const state=quizState();state.attempts.unshift({courseId:COURSE_ID,score,total:QUESTIONS.length,at:new Date().toISOString()});state.attempts=state.attempts.slice(0,20);write(QUIZ_KEY,state);addActivity(score);renderSummary();const pct=Math.round(score/QUESTIONS.length*100);result.hidden=false;result.innerHTML=`<p><strong>Score: ${score}/${QUESTIONS.length} (${pct}%).</strong> ${pct>=80?'Strong result. Review any missed explanation before moving on.':'Review the explanations, use the resources below, and retry when ready.'}</p><nav class="lms-quiz-actions" aria-label="Recommended RCA review resources"><span>Next study:</span><a class="btn btn-secondary" href="/guides/root-cause-analysis-5-why/">RCA and 5-Why guide</a><a class="btn btn-secondary" href="/guides/fmea-maintenance/">Maintenance FMEA guide</a></nav>`;result.focus();
    });
