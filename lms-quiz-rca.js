(()=>{
  ...
  const mount=()=>{
    ...
    form.addEventListener('submit',e=>{
      ...
      const state=quizState();state.attempts.unshift({courseId:COURSE_ID,score,total:QUESTIONS.length,at:new Date().toISOString()});state.attempts=state.attempts.slice(0,20);write(QUIZ_KEY,state);addActivity(score);renderSummary();const pct=Math.round(score/QUESTIONS.length*100);result.hidden=false;result.textContent=`Score: ${score}/${QUESTIONS.length} (${pct}%). ${pct>=80?'Strong result. Review any missed explanation before moving on.':'Review the explanations and the RCA guide, then retry when ready.'}`;result.focus();
    });
