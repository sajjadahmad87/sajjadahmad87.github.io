(()=>{
  ...
  const render=()=>{
    ...
    const recommendations=bySkill.map(s=>{
      ...
      return `<div class="lms-item"><div class="lms-item-icon">NEXT</div><div><h3>${esc(s.name)}</h3><p><strong>Suggested study method:</strong> ${esc(METHODS[chosen.key]||chosen.key)}</p><p style="margin-top:6px">Reason: ${s.score===null?'no current self-check score':`latest self-check ${s.score}% versus ${target}% study benchmark`}${s.priorityCount>=2?` · selected as a weekly priority ${s.priorityCount} times`:''}. Evidence: ${esc(evidence)} · average change <span class="${cls(chosen.avg)}">${fmt(chosen.avg)}</span> · ${esc(confidence)}.</p></div><div><a class="btn btn-secondary" href="#strategy-outcomes">Test this method</a></div></div>`;
    }).filter(Boolean);
    const recommendationHtml=recommendations.length?`<div style="margin-top:18px"><h3 style="font-size:14px">Evidence-gated next study method</h3><div class="lms-list">${recommendations.join('')}</div><p class="lms-local-note">A suggestion appears only when the topic is still a development gap and the browser has repeated positive strategy evidence. Topic-specific evidence is preferred; cross-topic evidence requires at least three experiments and is labelled more cautiously.</p></div>`:`<div class="lms-status" style="margin-top:14px"><strong>No automatic method suggestion yet.</strong> The LMS needs both a current development gap and sufficient repeated positive strategy evidence before recommending a study method.</div>`;
